...
int outputParam(int argc, char ** argv);
...
main(int argc, char ** argv)
{
  ...
  if (subcmd == "find")
    ...
    return outputParam(argc - 2, argv + 2);
...
int outputParam(int argc, char ** argv)
{
  ...
  for (std::size_t i = 1; i < positional.size(); i++)
  {
    ...
    // gather parameters
    hit::Node* targetNode = root->find(pattern.param);
    std::cout << targetNode->fullpath() << " has children:" << std::endl;
    for (auto& child : targetNode->children())
      std::cout<< child->path() << "_";
    std::cout << std::endl;
  }
}